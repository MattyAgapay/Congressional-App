...
function drawZones(){
  ...
  zones = L.geoJSON(reefs,{
    style:{ color:"#111", weight:1, fillOpacity:.5, fillColor:zoneColor() },
    onEachFeature:(f,l)=>{
      l.on("click",()=>{
...
function end(){
  const m = state.reefHealth>=75&&state.fishPop>=70
    ? "🌊 Reef thriving — amazing stewardship!"
    : state.reefHealth>=55
      ? "🌱 Mixed results — reef recovering."
      : "⚠️ Reef stressed — stronger kapu needed.";
  alert(m);
}
